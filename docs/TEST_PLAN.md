...
| # | Effort | Task | Deliverable |
|---|--------|------|-------------|
| 3.1 | 2-3 weeks + infra | **24-hour soak test** — `bench/soak.cr`. Alloc storm (1000 obj/s), periodic collect (1 Hz), fiber spawn (10 Hz), thread spawn (0.1 Hz), finalizer load (100 obj/s), WeakRef load. Hourly telemetry: heap size, pause stats, live_objects, RSS. After 24h: live_objects == 0, RSS within an **absolute** ceiling of start (`--rss-limit-kb`, default 4096 kB), no crash. The bound was a percentage and that was wrong: RSS here is warm-up plus 256 KiB chunk steps totalling ~1 MiB whether the run is 10 s or 4 h, and 10% of a ~6 MB base is three chunks. Run weekly via CI cron **at 5 h**, not 24: a GitHub-hosted job is cancelled at 6 h, so the 24 h arm never once completed there (2026-08-03 cancelled at 6h00m14s). 24 h is the local number — `make soak`. Dispatchable on demand (`workflow_dispatch`, with `monitor_gate` / `stall_ms` for A/B). | Soak test, CI cron |
