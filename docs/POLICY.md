...
| Resizing default context / extra parallel contexts | **Experimental** — `GCRY_TLAB=1` for alloc-side freelist locality; STW suspends Crystal OS threads. `GCRY_PARALLEL_MARK=N` (default **1**) uses STW-exempt raw pthreads; **not** a free thr win — Kemal/acikturkiye HTTP wrk regressed vs serial. Measure before enabling. |
