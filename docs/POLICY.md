...
| Resizing default context / extra parallel contexts | **Experimental** — enable `GCRY_TLAB=1` for alloc-side freelist locality; STW suspends Crystal OS threads. `GCRY_PARALLEL_MARK=N` uses STW-exempt raw pthreads for mark steals on process GC. Not tuned for high parallelism |
