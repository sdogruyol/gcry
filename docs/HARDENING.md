...
| `GCRY_INDEX_AUDIT=1` | **Research only**, and it does two things. It counts unlocked chunk-index reads taken during a stop, split by whether the reader is the thread that stopped the world — `chunk_containing` skips `@index_lock` while `@world_stopped` is set, on the grounds that only the collector can be there, and until 2026-08-22 `start_world` cleared that flag *after* resuming every thread (**173 326** foreign reads across 15 runs, 0 after). And it validates every chunk both lookup paths return, **refusing** an impossible one instead of handing it to `ChunkHeader.large?`: that refusal is what lets a run that would have crashed survive to say which path produced it. Counters `index_unlocked_owner` / `_foreign` / `_foreign_id`, `index_cache_bad`, `index_search_bad`, `index_cache_oob` on `/gc-stats`. `make stw-index-race`, `make find-block-race` |
