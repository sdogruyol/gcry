...
Process GC defaults (Phase 12): majors at **32 MiB** full STW; nursery off; size-class ceiling 32 KiB; **empty chunks released** unless `GCRY_KEEP_CHUNKS=1`; base-pointer-only mark. Auto-collect is suppressed while finalizers run (avoids nested collect).
