...
| `GCRY_THREAD_LIST_TRIPWIRE=1` | Research arm, default **off**. One unlocked walk of the runtime's `Thread.threads` immediately before `stop_world` takes `Thread.lock`, counting nodes — the `0x18` crash is that lock reading a null out of the list object, so this is the last instruction at which the damage can be reported as damage. Off by default because the walk owns a hazard of its own: a stale `next` from an exited worker can lead it into a legitimately released chunk. Measured 160/160 interleaved on `dormant_flush_race` under load: every on-arm `0x18` (4 of 4) was preceded by the tripwire's "held 7 threads, reads empty" line, and 26 more crashes moved from an unexplained `0x18` to a guard-named fault *inside the walk*, in a large-object chunk released one sweep earlier — see `bench/log/linux/2026-08-27-thread-list-tripwire/` |
