...
- Free-page physical release: **opt-in on both platforms** (`GCRY_PAGE_DONTNEED=1`). Linux HOLED was already opt-in (measured thr+RSS regression as default); Darwin shipped it **on** until 0.21.0 and is off now to match. The walk was unsound until 2026-09-04 and is sound now (it runs under every small-allocation lock) — see the knob table below. Escape: `GCRY_DISABLE_PAGE_RELEASE=1` / `GCRY_DISABLE_MADVISE=1`
...
| `GCRY_PAGE_DONTNEED=1` | Sparse free-page release, **opt-in on Linux and Darwin alike** (Darwin was default-on before 0.21.0). Sound since 2026-09-04: the unlink, the free-page mask, and the `madvise` run inside `with_small_allocation_excluded` (every TLAB and batch slot lock, then the class freelist lock), so no block in the run can be handed out until the syscall returns. Before that a TLAB-held block written in between was zeroed (`make page-release-corruption` faulted 4 of 28); after, 0 of 12 there and 0 of 30 on `make live-graph-audit`. Off by default for throughput and RSS, not soundness. |
...
| `GCRY_SCRUB_LIBC_BOUNDS=1` | **Research only.** Take the collector scrub's stack bounds from `pthread_getattr_np` instead of `Fiber#@stack`, as gcry did until 2026-09-04. For the *initial* thread glibc answers that call by parsing `/proc/self/maps`, and the scrub runs twice per collection, outside the pause window — so the cost was invisible to `pause_p50`/`p99` and grew with the mapping count: measured 15.7 µs a call at 48 maps lines, **2.5 ms** at 4 000 parked fibers, **24.7 ms** at 20 000. `clear_stack_libc_bounds` counts the fallbacks and must read 0. `make collect-scrub-cost` |
| `GCRY_MARK_BUSY_UNLOCKED=1` | **Research only.** Count a parallel-mark worker busy *after* releasing the lock that gave it the batch, as gcry did until 2026-09-04, with the window widened. The master's `busy == 0 && stack empty` stop condition then fires while a worker still holds up to `MARK_POP_BATCH` unscanned objects, so their children are never marked and are swept **while live** — the red arm loses a live object on the first collection. Never ship non-zero. `make parallel-mark-termination` |
