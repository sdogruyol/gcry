...
module Gcry
  ...
  module Platform
    ...
    # holding `@@data_anchor` *is* the executable's `.data` and the one holding
    # `@@bss_anchor` *is* its BSS — by address, with no pathname to compare, no
    # `/proc/self/exe` to resolve and no line order to trust. Verified on
    # Crystal 1.21 / LLVM 22 with `nm`: `D` and `B` respectively.
    @@data_anchor : UInt64 = 0x6772_6379_u64
    @@bss_anchor : UInt64 = 0_u64

    private def self.data_anchor : UInt64
      pointerof(@@data_anchor).address
    end

    private def self.bss_anchor : UInt64
      pointerof(@@bss_anchor).address
    end
