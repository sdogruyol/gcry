...
soak-smoke: $(BIN)
	...
	# warmup=0: CI smoke is 10s; baseline is cold RSS (looser --rss-limit covers ramp).
	$(BIN)/soak --duration=10 --warmup=0 --rss-limit=$${SOAK_SMOKE_RSS_LIMIT:-30} --telemetry=/tmp/gcry-soak-smoke.log
