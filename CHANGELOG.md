...
- **EC4 post-STW queue:** SpinLock wait on `@post_stw` burned ~8–11s/20s of worker time under Parallel HTTP. Switch to embedded `pthread_mutex`; auto-collect **coalesce** when a peer already cleared the debt; pause stats exclude queue wait. EC4 `/json` ~**40k** med (d=20) + soak **20/20** (was ~22k + crash outliers). See FINDINGS.
