...
- **No collect during Parallel thread bootstrap:** `Thread#start` → `Fiber::new` → malloc can hit `maybe_collect` before `@current_fiber` is set; process STW then dies with `Thread#current_fiber cannot be nil` / `END_OF_STACK` (Kemal EC4 + `GCRY_THRESHOLD=32KiB` at boot). Skip collect while the current thread has a nil `@current_fiber`.
