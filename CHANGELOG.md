...
- Re-recorded same-host median-of-3 (scrub off): Kemal `/` **~89%** / `/json` **~92%** of Boehm, post-GC RSS **~0.97×** ([docs/PERF.md](docs/PERF.md)); acikturkiye `/api/v1/` thr trial-median **~93%**, post-GC RSS **~2.84×** (was ~3.20× at 0.8.0) — [docs/ACIKTURKIYE.md](docs/ACIKTURKIYE.md). Helpers: `bench/median_kemal_boehm.sh`, `bench/median_acikturkiye_boehm.sh`.
- Prometheus: `gcry_clear_stack_*` / `gcry_fiber_scrub_*` counters (with `json_stats`).
