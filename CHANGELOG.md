...
- **Property-based testing:** `bench/property_test.cr` — random alloc/free/collect sequences with invariant verification (live_objects, freelist consistency). Uses `collect(roots:)` with all live pointers as roots (proven pattern from bench/fuzz.cr). Enables GCRY_DEBUG_INVARIANTS=1 for extra validation on every collect. 100k iterations in ~7s. (`make property-test PROP_ITERATIONS=100000`, CI `Property test` step.)
