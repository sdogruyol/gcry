...
- **Multi-mutator STW stack LAG:** full `guard→bottom` on every parked fiber dominated EC4 `phase_roots` (~100ms+/collect). Prefer suspend SP−red_zone when present; otherwise scan from `stack_top − 512KiB` (not full guard). Same-host A/B `/json` median-of-5: LAG **~30k** vs stw_full **~16k** (~1.9×); EC4 soak 30×8s **0/30**. EC1 clamp path unchanged. See FINDINGS.
