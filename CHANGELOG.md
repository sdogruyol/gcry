...
|- **API misuse test suite:** `spec/api_misuse_spec.cr` -- tests covering `GC.free(null)`, `GC.realloc(null, 0)`, `GC.malloc(0)`, `GC.malloc_atomic(0)`, `Gcry.add_root(null)`, `Gcry.register_disappearing_link(null, ...)`, `GC.collect` inside signal handler (Linux), `add_root` with large pointer, alternating malloc/free. (`make spec`, CI `spec` step.)
...
|- **Perf regression alerting:** `bench/perf_smoke.sh` rewritten with variance protocol -- 5 wrk runs per path, min/max discarded, median reported, noise ratio computed (IQR/median). CI compares gcry against historical baseline from `bench/baseline.json`; alerts on >5% /json drop when noise < 0.15. Results stored in `bench/log/` per run, uploaded as CI artifact. (`bench/perf_smoke.sh`, `bench/baseline.json`, CI `perf smoke` job.)
- **Microbenchmark suite:**  -- 6-phase suite measuring alloc latency (10 size classes, p50/p99/max), free latency, collect latency (5000 obj, p50/p99/max), TLAB refill cost, STW suspend/resume latency, and GC lock overhead. Runs in < 10s. (, CI  step.)
