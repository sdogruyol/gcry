...
#
# Both tables now live in `Gcry::StwSlots` and grow at collection entry, before
# the first suspend, with `LibC.malloc` and never inside the stopped world
# (`malloc` there is the 2026-08-10 six-hour hang). The 64-bit claim mask is
# gone — it *was* the bound, since a `UInt64` cannot address a 65th slot —
# replaced by one byte per slot, with no CAS because on these two platforms
# `slot_for` runs only on the collector, one thread at a time.
#
# The table is shared rather than per-platform for a reason this gate cannot
# supply on its own: a first attempt grew the two platform copies in place and
# crashed the Darwin job, and nobody here can run Darwin. `spec/stw_slots_spec.cr`
# now covers the growth, the claiming and a reader walking the table *while* it
# grows, on whatever platform runs the suite — and that example faults within
# three runs if the growth frees its predecessor, which is what the crash was.
#
